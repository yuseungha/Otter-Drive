...
class TrafficMotorController:
    ...
    no-speed-feedback fallback delay. Automatic GO and TURN LEFT use the full
    normalized ESC command; manual mode passes the operator throttle through.
    """
    ...
    def __init__(
        ...
        self.left_steering = self._normalize_steering(left_steering)
        self.gear = int(gear)
    ...
    @staticmethod
    def _normalize_throttle(value: int) -> int:
        return max(THROTTLE_MIN, min(THROTTLE_MAX, int(value)))

    @staticmethod
    def _normalize_steering(value: int) -> int:
    ...
    def update_manual_command(
        ...
        self.manual_throttle = self._normalize_throttle(throttle)
        self.manual_steering = self._normalize_steering(steering)
    ...
    def command(self) -> Optional[DriveCommand]:
        ...
        if self.signal == 'TURN LEFT':
            if self.manual_mode:
                target_throttle = max(0, self.manual_throttle)
                reason = 'manual_turn_left'
            else:
                target_throttle = THROTTLE_MAX
                reason = 'turn_left'
            ...
            if self.manual_mode:
                ...
                self.current_steering = 0
                reason = 'go_straight'
        self.current_throttle = float(target_throttle)
        return DriveCommand(
            target_throttle,
            self.current_steering,
            self.gear,
