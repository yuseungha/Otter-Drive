"""ROS adapter for center-path planning from YOLO segmentation masks."""
...
class YoloSegmentationLaneNode(Node):
    """Infer lane masks, fit an image-space path, and publish safe controls."""

    def __init__(self) -> None:
        super().__init__('yolo_seg_lane_detector')
        ...
        self.get_logger().info(
            f'YOLO segmentation lane planner ready: {model_path} | '
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter('look_ahead_ratio', 0.74)
    ...
    def _draw_overlay(
        ...
        overlay = bgr.copy()
        for instance in instances:
            pixels = instance.mask >= self.planner.config.mask_threshold
            color = np.asarray(
                (255, 180, 30)
        ...
        for row in geometry.get('scan_rows', []):
            ...
            for key, color in (
                ...
                if value is not None:
                    cv2.circle(
                        overlay, (int(round(value)), y), 3, color, -1)
        path = [
            (int(round(point['x'])), int(point['y']))
        ...
        if target_x is not None:
            ...
            cv2.circle(
                overlay, (int(round(target_x)), target_y),
                7, (0, 0, 255), 2)
            cv2.line(
                overlay, (center_x, target_y),
                (int(round(target_x)), target_y), (0, 0, 255), 2)
    ...
    def _run_inference(self) -> None:
        ...
        try:
            ...
            binary = self._combined_mask(instances, bgr.shape)
            overlay = self._draw_overlay(
                bgr, instances, geometry, inference_ms)
            ...
            details = []
            annotated = bgr.copy()
            binary = np.zeros(bgr.shape[:2], dtype=np.uint8)
            overlay = self._draw_overlay(
                bgr, [], geometry, inference_ms)
        self._publish_plan(
