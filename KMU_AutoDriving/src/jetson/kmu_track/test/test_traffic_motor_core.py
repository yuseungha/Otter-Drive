...
def test_go_applies_full_forward_with_centered_steering() -> None:
    clock = FakeClock()
    controller = _controller(clock)
    controller.update_signal('GO')
    clock.advance(0.25)
    command = controller.command()
    assert command.throttle == 1000
    assert command.steering == 0
    assert command.gear == -1


def test_turn_left_keeps_full_throttle_and_positive_steering() -> None:
    clock = FakeClock()
    controller = _controller(clock)
...
def test_manual_go_passes_operator_command_without_ramp() -> None:
    clock = FakeClock()
    controller = _controller(
        clock,
        manual_mode=True,
...
def test_manual_red_stops_then_centers_and_green_resumes() -> None:
    ...
    assert resumed.throttle == 180
    assert resumed.steering == 90
    assert resumed.reason == 'manual_go'


def test_manual_left_arrow_keeps_speed_and_overrides_steering() -> None:
    clock = FakeClock()
    controller = _controller(
        clock,
        manual_mode=True,
