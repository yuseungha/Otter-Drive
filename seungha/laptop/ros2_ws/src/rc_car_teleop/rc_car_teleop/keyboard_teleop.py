...
=== RC Car keyboard control ===
 W/S : throttle + / - (value persists until changed or stopped)
 A/D : steer left / right
 1   : LOW gear           2: HIGH gear
...
Stop the throttle before selecting LOW or HIGH gear.
Throttle range: -1000 .. 1050. Steering range: -1000 .. 1000.
...
class KeyboardTeleop(Node):
    ...
    def render_status(self, force=False):
        ...
        with self._print_lock:
            ...
            signal, throttle, steering, gear_value, output_throttle, \
                output_steering, reason = snapshot
            gear = 'LOW' if gear_value == GEAR_LOW else 'HIGH'
            print(
                f'\rCAM={signal:9s} | '
                f'INPUT T={throttle:5d} S={steering:5d} {gear:4s} | '
    ...
    def publish_command(self):
        with self._lock:
            values = [self._throttle, self._steering, self._gear]
    ...
    def handle_key(self, key):
        ...
        with self._lock:
            if key in ('w', 'W'):
                self._throttle = min(
                    THROTTLE_MAX, self._throttle + THROTTLE_STEP)
            elif key in ('s', 'S'):
                self._throttle = max(
                    THROTTLE_MIN, self._throttle - THROTTLE_STEP)
            elif key in ('a', 'A'):
                self._steering = min(
