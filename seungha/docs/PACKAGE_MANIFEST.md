...
- `laptop/ros2_ws/src/kmu_track`
  - 신호등·좌회전 화살표 검출
  - 신호등 상태 기반 모터 안전 게이트
  - USB 카메라, 미리보기 서버, 기존 미션·차선 노드
  - 설정, launch, 단위 테스트
- `laptop/ros2_ws/src/rc_car_teleop`
  - 키보드 수동 입력
  - Arduino 직렬 브리지
  - launch, 단위 시험, 기어·서보 안내
- `laptop/ros2_ws/src/laptop_teleop`
  - 웹 수동 입력과 단일 운전자·heartbeat 상태 머신
  - 웹 UI, launch, 실행·검증 스크립트
  - Arduino 안전 펌웨어와 조향 피드백 도구
- `laptop/ros2_ws/tools/traffic_light_camera_test.py`
- `laptop/windows_launchers`
...
압축을 푼 뒤 `laptop/ros2_ws`에서 바로 `colcon build`할 수 있는 구조를 유지합니다.
